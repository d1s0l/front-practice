...
- Мини-игры:
  quick duel встроен в session-layer как короткий вопросный режим, а полноценный pong вынесен в отдельный feature-layer hook [usePingPongGame.ts](../features/ping-pong/model/usePingPongGame.ts) и overlay-компонент [PingPongOverlay.tsx](../features/ping-pong/ui/PingPongOverlay.tsx).
...
- Мир и декоративные объекты:
  [world-objects.ts](../shared/config/world-objects.ts) хранит параметры расширенной комнаты, кота-компаньона, IT-постеров и pong-уголка рядом с NPC скорости.
...
- Pong-матч живёт как отдельный overlay-режим поверх комнаты, поэтому управление персонажем, движение по сцене и обычные диалоги не конфликтуют с логикой аркадного режима.
...
- Pong-стол и arcade-уголок остаются частью 3D-сцены, а сама игровая механика матча живёт в отдельном overlay-режиме поверх комнаты.
